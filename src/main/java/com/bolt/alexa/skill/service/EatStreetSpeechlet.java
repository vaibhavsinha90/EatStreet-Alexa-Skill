...
public class EatStreetSpeechlet implements SpeechletV2 {
	...
	public SpeechletResponse onIntent(SpeechletRequestEnvelope<IntentRequest> speechletRequestEnvelope) {
		...
		case "FindFood":
			/*
			 * log.info("user here: "+session.getUser().toString()); log.info(
			 * "userID here: "+session.getUser().getUserId().toString());
			 * log.info("user perms here: "
			 * +session.getUser().getPermissions().toString());
			 */
			...
			if (consentToken == null) {
				...
				try {
					...
					AlexaDeviceAddressClient alexaDeviceAddressClient = new AlexaDeviceAddressClient(deviceId,
							consentToken, apiEndpoint);
		...
		case "AMAZON.HelpIntent":
			return getAskResponse(
					"You can find restaurants by cuisine, or ask if a restaurant is open/will deliver to your house.",
					HELP_TEXT);
	...
	private SpeechletResponse getRestaurantSearchResponse(Intent intent, Address deviceAddress) {
		...
		if (findFoodIntent) {
			...
			if (findFood == null)
				return getAskResponse(
						"You can find restaurants by cuisine, or ask if a restaurant is open/will deliver to your house.",
						"You did not provide a search term. " + RePrompt);
		} else {
			...
			if (findRestaurant == null)
				return getAskResponse(
						"You can find restaurants by cuisine, or ask if a restaurant is open/will deliver to your house.",
						"You did not provide a restaurant name. " + RePrompt);
		}

		log.info("Device Address= " + deviceAddressText + "will check eatstreet now");
		RestaurantSearchResponse response = restaurantSearchService.search(findRestaurant, findFood, deviceAddressText,
				defaultRadius, checkDelivery);
